...
- [x] `TALK` takes input (e.g. `talk "hi"`) with random responses — modular for future growth ([#36](https://github.com/CryptoJones/ChromeCircuitCowboys/issues/36)) — _shipped v2.5.0: TALK <words> gets an intent-routed reply (greet/bye/thanks/insult/ask/smalltalk) from the room's NPC or local_
- [x] AI characters that act as players to populate empty servers (so solo players aren't lonely) ([#37](https://github.com/CryptoJones/ChromeCircuitCowboys/issues/37)) — _shipped v2.5.0: AI runners wander the surface + chatter near humans, show in WHO; never fight/save/rank; `-bots` flag_
- [x] Use inventory item numbers as command targets — `SELL N` / `LOOK N` / `GIVE N` / `DROP N` etc. where N is the `INV` item number (modular across all item commands) ([#58](https://github.com/CryptoJones/ChromeCircuitCowboys/issues/58)) — _shipped v2.5.0: shared resolveInvItem wires SELL/USE/LOOK/GIVE/DROP to the numbered inventory_
